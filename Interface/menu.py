import os
from Servicos.arquivo_service import adicionar_historico
from Servicos.estoque_service import procurar_produto_nalista, verificar_se_ha_produtos, cadastrar_produto, excluir_produto, entrada_estoque, saida_estoque, editar_produto


def mostrar_menu(Estoque, Histórico): #Mostra o menu de opções
        print(
        "Estoque\n"
        "1-Listar Produtos\n""2-Cadastrar Produto\n"  "3-Excluir Produto\n"  "4-Entrada de estoque\n"
        "5-Saída de estoque\n"   "6-Editar Produto\n" "7-Histórico de entrada/saída de estoque\n"   "8-Sair"
    )
        escolher_funcionalidade(Estoque, Histórico)

def limpar_terminal(): # Limpa o terminal.
    input("Pressione qualquer tecla para recomeçar")
    os.system("cls" if os.name == 'nt' else "clear")

def escolher_funcionalidade(Estoque, Histórico): # Permite o usuário escolher qual funcionalidade deseja acessar.
    try:
        ...
        match finalidade:
            case 1:
                ...
                limpar_terminal()
            case 2:
                valores = obter_produto_cadastrado(Estoque)
                cadastrar_produto(Estoque, valores[0], valores[1], valores[2], valores[3]) if valores is not None else ''
            case 3:
                listar_produtos(Estoque)
                produto = obter_produto_excluido(Estoque)
                excluir_produto(Estoque, produto) if produto is not None else ''
            case 4:
                listar_produtos(Estoque)
...
def listar_produtos(Estoque): # Lista os produtos no estoque
    if verificar_se_ha_produtos(Estoque):
...
def obter_produto_cadastrado(Estoque):
    nome_novo_produto = input("Qual nome do produto?  ")
    verificacao_se_existe = procurar_produto_nalista(Estoque, nome_novo_produto)
...
def obter_produto_excluido(Estoque): # Permite que o usuário exclua um produto cadastrado
        produto_futuramente_excluido = input("Qual o nome do produto?  ")
        produto_excluido = procurar_produto_nalista(Estoque, produto_futuramente_excluido)
...
def obter_entrada_estoque(Estoque):#Permite aumentar a quantidade de um produto
    produto_futuramente_adicao_estoque = input("Qual produto você quer adicionar quantidade?  ")
    produto_adicao_estoque = procurar_produto_nalista(Estoque, produto_futuramente_adicao_estoque)
...
def obter_saida_estoque(Estoque):# Permite diminuir a quantidade de um produto
    produto_futuramente_subtracao_estoque = input("Qual produto você quer retirar quantidade?  ")
    produto_subtracao_estoque = procurar_produto_nalista(Estoque, produto_futuramente_subtracao_estoque)
    if produto_subtracao_estoque is not None:
        try:
            ...
            if quantidade_retirada > 0 and produto_subtracao_estoque.quantidade >= quantidade_retirada:
                print(f"A quantia foi retirada, agora há {(produto_subtracao_estoque.quantidade - quantidade_retirada)} unidades de {produto_subtracao_estoque.nome}!")
                limpar_terminal()
...
def obter_produto_editado(Estoque):#Permite editar informações de um produto
    produto_futuramente_editado = input("Qual Produto deseja editar?  ")
    produto_editado = procurar_produto_nalista(Estoque, produto_futuramente_editado)
...
def escolha_filtrohistorico(Histórico):#Escolhe o estilo do filtro
        ...
        match escolha:
            case 1:
                if len(Histórico) > 0:
                    ...
                    return
            case 2:
                procurar_historico_produto(Histórico)
                limpar_terminal()
                return
            case _:
...
def procurar_historico_produto(Histórico): #Procura o histórico do produto
    ...
    match operacao:
        case '+' | '-' | '/' :
            mostrar_historico(Histórico, produto, operacao)
        case _:
            print("❌ Só aceitamos os sinais, tente novamente")
            limpar_terminal()
            return
        
def mostrar_historico(Histórico, produto, operação): #Mostra o estoque do produto e operação escolhida por usuário
    if len(Histórico) > 0:
        verificacao = None
        for registro in Histórico:
